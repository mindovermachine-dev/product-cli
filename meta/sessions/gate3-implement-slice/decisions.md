...
Fifty-one, of which **one is withdrawn, one reversed twice, one deleted outright, one superseded, one retired, and one settled by a later ruling**. Each is marked at its site in the source with the same `D-nn` tag
...
| D-52 | DECIDED | `DeliveryAssurance` is **ordered** (`Unassured < Enqueued < Dispatched < Confirmed`) so a requirement can be compared against a ceiling. The ordering is this session's; nothing states that assurance levels are totally ordered, and a scheme with per-consumer acknowledgement semantics might not be. | `DeliveryAssuranceModel.cs` |
