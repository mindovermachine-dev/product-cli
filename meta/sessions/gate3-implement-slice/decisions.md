...
| D-51 | DECIDED | **The assurance ceiling is derived from `consumption_observable`; the requirement is not derived from anything.** Inferring how sure we need to be from what we can observe would be the regex mistake one field over — a capability claim is not a requirement claim. `ReadRequiredAssurance` looks for a field, finds none, and reports unattributed. | `CarrierModel.cs`, `DeliveryAssuranceModel.cs` |
| D-53 | DECIDED | A profile's **actual** discharge is derived from its mechanism, never from its declaration. A profile declaring `confirmed` over a synchronous call discharges `Unassured`, and the declaration is the thing that is wrong. Checked before the act's requirement is consulted. | `ProfileDischargeModel.Overclaims` |
| D-54 | DECIDED | `delivery_assurance` is read for **write** positions only, not reads. "Everywhere" is R-Q45's word and its edge is unstated; "reaching the external system" is a delivery phrase. Q-46. | `ReadRequiredAssuranceForEveryWrite` |
| D-52 | DECIDED | `DeliveryAssurance` is **ordered** — **affirmed by R-Q45** ("the levels here matter") (`Unassured < Enqueued < Dispatched < Confirmed`) so a requirement can be compared against a ceiling. The ordering is this session's; nothing states that assurance levels are totally ordered, and a scheme with per-consumer acknowledgement semantics might not be. | `DeliveryAssuranceModel.cs` |
